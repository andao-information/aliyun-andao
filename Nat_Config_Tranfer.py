...
class Get_Nat_Config:
    ...
    def __init__(self,NatGatewayId = '',new_NatGatewayId='',RegionId = '' , accessKeyId = '' ,accessSecret = '', domain = ''):
        ...
        #========================================================================#
        self.new_NatGatewayId = new_NatGatewayId
        
        self.new_nat_config = self.DescribeNatGateway(self.new_NatGatewayId)
        #========================================================================#
    ...
    def Create_Route_ntry(self):
        while True:
            ...
            if 'Message' in tmp_json:
                ...
                print(tmp_json)
                break
def run(NatGatewayId,new_NatGatewayId,RegionId,accessKeyId,accessSecret,domain):
    test = Get_Nat_Config(NatGatewayId = NatGatewayId,
    new_NatGatewayId = new_NatGatewayId,
...
if __name__ == '__main__':
    opts, args = getopt.getopt(sys.argv[1:], '-hn:-N:-r:-k:-s:-d', ['help',\
                                                              'NatGatewayId=',\
                                                              'new_NatGatewayId=',\
                                                              'RegionId=',\
                                                              'accessKeyId=',\
    ...
    new_NatGatewayId = ''#ngw-8vb60f6e5ys71ddysp0il
    ...
    for key, value in opts:
        if key in ['-h', '--help']:
            print('联系人：陈柳安')
            print('处理新旧NAT配置转换，配置包括SNAT，EIP，NAT路由条目。')
            print('确保创建新旧NAT处于同一交换机、VPC、路由表、地域、账号。')
            print('请勿新旧NatId 填反，避免新Nat配置跟新到旧Nat配置。导致配置丢失')
            print('记得保存旧NAT配置文件。')
            print('参数：')
            print('-h\t--help\t\t\t显示帮助')
            print('-n\t--NatGatewayId\t\t输入你的旧NAT网关ID')
            print('-N\t--NewNatGatewayId\t输入你新创建的NAT网关ID')
            print('-r\t--RegionId\t\t输入地域')
            print('-k\t--accessKeyId\t\t输入accessKeyID')
            print('-s\t--accessSecret\t\t输入accessSecret')
            print('-d\t--domain\t\t输入请求的VPC域名')
            
            sys.exit(0)
            
        if key in ['-n', '--NatGatewayId']:
            NatGatewayId = value
        if key in ['-N', '--NewNatGatewayId']:
            new_NatGatewayId = value
    ...
    print (' NatGatewayId：\t\t'+NatGatewayId, \
           '\n','NewNatGatewayId:\t'+new_NatGatewayId, \
           '\n','RegionId：\t\t'+ RegionId, \
            '\n', 'accessKeyId：\t\t'+accessKeyId,\
              '\n', 'accessSecret：\t\t'+accessSecret,\
               '\n',  'domain：\t\t'+domain )
    print('===================================')
    print('请勿新旧NatId 填反，避免新Nat配置跟新到旧Nat配置，请确认参数后再开始')
    print('输入：Y    开始执行')
    print('===================================')
    str_y = input("请输入：")
    if str_y == 'Y' or str_y == 'y':
        print('STAR')
        run(NatGatewayId,new_NatGatewayId,RegionId,accessKeyId,accessSecret,domain)
        sys.exit(0)
        

 
'''test
NatGatewayId = 'ngw-8vbp1n8hws2lm0fcl9p89'
new_NatGatewayId = 'ngw-8vb60f6e5ys71ddysp0il'
RegionId = 'cn-zhangjiakou'
accessKeyId = 'LTAI5tPC6VDhND92eN8hqwWy'
accessSecret = 'q8rrb2aQPAjYCnHUdr0sYBzaDEXVLD'
domain = 'vpc.cn-zhangjiakou.aliyuncs.com'
run(NatGatewayId,new_NatGatewayId,RegionId,accessKeyId,accessSecret,domain)
'''
